...
When I deployed to *Heroku*, the input fields on the login and signup pages did not show the invisible text. Locally the text was black after updating my CSS, but in production it stayed transparent. Checking the deployed CSS with `heroku run "cat staticfiles/css/style.css | grep color"` showed that `color: #0000;` was still present. This meant old CSS was being served by *Heroku*, even though I had already fixed it in my local project.

**Fix:**

I discovered that the problem was caused by old files inside the `staticfiles/` directory. This folder is generated by `collectstatic` and should never be version-controlled, but older copies had slipped into *Heroku*’s cache.  

Steps I followed to fix it:

1. Removed any `staticfiles/` directory from Git so it would not be tracked:
    In `powershell`
    git rm -r --cached staticfiles
    echo "staticfiles/" >> .gitignore
    git add .gitignore
    git commit -m "Remove staticfiles dir from version control"
    git push heroku main

2. Opened a bash shell on *Heroku* and deleted the existing collected static files:
    In `bash`
    heroku run bash
    rm -rf /app/staticfiles/*
    exit
    
3. Forced a clean rebuild on *Heroku* to ensure no old files were cached:
    In `powershell`
    git commit --allow-empty -m "Force clean rebuild on Heroku"
    git push heroku main
    
4. Re-ran collectstatic to regenerate fresh static files from my updated CSS:
    In `powershell`
    heroku run "python manage.py collectstatic --noinput"

5. Verified the fix by searching again:
    In `powershell`
...
Never track `staticfiles/` in *Git* — it should only exist on the server after `collectstatic`. If old rules show up in production, it usually means stale static files are being cached on *Heroku*. The fix is to wipe `/app/staticfiles`, force a rebuild, and regenerate clean static files. Always confirm with a `grep` check to ensure the deployed CSS matches local changes.
